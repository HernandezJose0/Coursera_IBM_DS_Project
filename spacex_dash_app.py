...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.RangeSlider(id='payload-slider',min=min_payload,max=max_payload,step=1000,value=[min,max]),
...
def get_pie_chart(entered_site):
    df=spacex_df
    if entered_site == "ALL":
        fig=px.pie(df,values="class",names="Launch Site",title="Success launches by site")
        return fig
    else:
        df["class-0"]=df["class"].apply(lambda x:1 if x == 0 else 0)
        df=df[df["Launch Site"]==entered_site]
        vals=[df["class-0"].sum(),df["class"].sum()]
        nms=["failed","success"]
        fig=px.pie(df,values=vals,names=nms,title="Success launches for "+entered_site)
        return fig
# TASK 4:
...
def get_scatter_plot(entered_site,payload):
    ...
    if entered_site == "ALL":
        ...
        fig=px.scatter(df[df["Launch Site"]==entered_site],x="Payload Mass (kg)",y="class",color="Booster Version Category")
        return fig
